...
class NicoRosMotion():
    ...
    def __init__(self, config=None):
        ...
        if rospy.has_param(config['rostopicName'] + '/vrepScene'):
            config['vrepScene'] = rospy.get_param(
                config['rostopicName'] + '/vrepScene')
        ...
        self.robot = Motion(motorConfig=config['robotMotorFile'], vrep=config['vrep'],
                            vrepHost=config['vrepHost'], vrepPort=config['vrepPort'], vrepScene=config['vrepScene'])
        ...
        rospy.Subscriber(
            '%s/closeHand' % config['rostopicName'], nicomsg.msg.s, self._ROSPY_closeHand)
        rospy.Subscriber('%s/enableForceControlAll' %
                         config['rostopicName'], nicomsg.msg.i, self._ROSPY_enableForceControlAll)
        rospy.Subscriber('%s/disableForceControlAll' %
                         config['rostopicName'], nicomsg.msg.empty, self._ROSPY_disableForceControlAll)
        rospy.Subscriber('%s/enableForceControl' %
                         config['rostopicName'], nicomsg.msg.si, self._ROSPY_enableForceControl)
        rospy.Subscriber('%s/disableForceControl' %
                         config['rostopicName'], nicomsg.msg.s, self._ROSPY_disableForceControl)
        rospy.Subscriber(
            '%s/setAngle' % config['rostopicName'], nicomsg.msg.sff, self._ROSPY_setAngle)
        rospy.Subscriber(
            '%s/changeAngle' % config['rostopicName'], nicomsg.msg.sff, self._ROSPY_changeAngle)
        rospy.Subscriber('%s/setMaximumSpeed' %
                         config['rostopicName'], nicomsg.msg.f, self._ROSPY_setMaximumSpeed)
        rospy.Subscriber('%s/setStiffness' %
                         config['rostopicName'], nicomsg.msg.sf, self._ROSPY_setStiffness)
        rospy.Subscriber(
            '%s/setPID' % config['rostopicName'], nicomsg.msg.sfff, self._ROSPY_setPID)
        rospy.Subscriber('%s/enableTorque' %
                         config['rostopicName'], nicomsg.msg.s, self._ROSPY__enableTorque)
        rospy.Subscriber('%s/disableTorque' %
                         config['rostopicName'], nicomsg.msg.s, self._ROSPY__disableTorque)
        rospy.Subscriber('%s/enableTorqueAll' %
                         config['rostopicName'], nicomsg.msg.empty, self._ROSPY__enableTorqueAll)
        rospy.Subscriber('%s/disableTorqueAll' %
                         config['rostopicName'], nicomsg.msg.empty, self._ROSPY__disableTorqueAll)
        rospy.Subscriber('%s/toSafePosition' %
                         config['rostopicName'], nicomsg.msg.empty, self._ROSPY__toSafePosition)

        # setup services
        logging.debug('Init services')
        rospy.Service('%s/getConfig' %
                      config['rostopicName'], nicomsg.srv.GetString, self._ROSPY_getConfig)
        rospy.Service(
            '%s/getVrep' % config['rostopicName'], nicomsg.srv.GetString, self._ROSPY_getVrep)
        rospy.Service(
            '%s/getAngle' % config['rostopicName'], nicomsg.srv.GetValue, self._ROSPY_getAngle)
        rospy.Service(
            '%s/getPose' % config['rostopicName'], nicomsg.srv.GetValues, self._ROSPY_getPose)
        rospy.Service('%s/getJointNames' %
                      config['rostopicName'], nicomsg.srv.GetNames, self._ROSPY_getJointNames)
        rospy.Service('%s/getAngleUpperLimit' %
                      config['rostopicName'], nicomsg.srv.GetValue, self._ROSPY_getAngleUpperLimit)
        rospy.Service('%s/getAngleLowerLimit' %
                      config['rostopicName'], nicomsg.srv.GetValue, self._ROSPY_getAngleLowerLimit)
        rospy.Service('%s/getTorqueLimit' %
                      config['rostopicName'], nicomsg.srv.GetValue, self._ROSPY_getTorqueLimit)
        rospy.Service('%s/getTemperature' %
                      config['rostopicName'], nicomsg.srv.GetValue, self._ROSPY_getTemperature)
        rospy.Service('%s/getCurrent' %
                      config['rostopicName'], nicomsg.srv.GetValue, self._ROSPY_getCurrent)
        rospy.Service('%s/getStiffness' %
                      config['rostopicName'], nicomsg.srv.GetValue, self._ROSPY_getStiffness)
        rospy.Service(
        ...
        self._jointStatePublisher = rospy.Publisher(
            '%s' % config['jointStateName'], sensor_msgs.msg.JointState, queue_size=1)
    ...
    def _ROSPY_setPID(self, message):
        ...
        """
        self.robot.setPID(message.param1, message.param2,
                          message.param3, message.param4)
    ...
    def _sendJointState(self):
        ...
        while self._running:
            ...
            joints_without_motor = ['l_indexfinger_1st_x', 'l_indexfinger_2nd_x', 'l_ringfingers_x', 'l_ringfinger_1st_x', 'l_ringfinger_2nd_x', 'l_thumb_1st_x',
                                    'l_thumb_2nd_x', 'r_indexfinger_1st_x', 'r_indexfinger_2nd_x', 'r_ringfingers_x', 'r_ringfinger_1st_x', 'r_ringfinger_2nd_x', 'r_thumb_1st_x', 'r_thumb_2nd_x']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log-file', dest='logFile',
                        help='Path to log file. Default: %s' % config['logFile'], type=str)
    parser.add_argument('-m', '--motor-file', dest='robotMotorFile',
                        help='Path to robot motor file. Default: %s' % config['robotMotorFile'], type=str)
    parser.add_argument('-v', '--vrep', dest='vrep',
                        help='Connect to VREP rather than to a real robot', action='store_true')
    parser.add_argument('--vrep-host', dest='vrepHost',
                        help='Host of VREP. Default: %s' % config['vrepHost'], type=str)
    parser.add_argument('--vrep-port', dest='vrepPort',
                        help='Port of VREP. Default: %i' % config['vrepPort'], type=int)
    parser.add_argument('--vrep-scene', dest='vrepScene',
                        help='Scene to load in VREP. Default: %s' % config['vrepScene'], type=str)
    parser.add_argument('--rostopic-name', dest='rostopicName',
                        help='Topic name for ROS. Default: %s' % config['rostopicName'], type=str)
    parser.add_argument('-j', '--joint-state-name', dest='jointStateName',
                        help='Name of the joint state topic. PLEASE NOTE: A lot of other nodes assume the joint state note to be at /joint_states, so be careful when changing it. Default: %s' % config['jointStateName'], type=str)
    parser.add_argument('-f', '--fake-execution', dest='fake',
                        help='Publish fake joint states instead of real joint states', action='store_false')
