...
class NicoRosMotion():
    ...
    def __init__(self, config=None):
        ...
        rospy.Service('%s/getConfig' %
                      config['rostopicName'], nicomsg.srv.GetString,
                      self._ROSPY_getConfig)
        rospy.Service(
            '%s/getVrep' % config['rostopicName'], nicomsg.srv.GetString,
            self._ROSPY_getVrep)
        rospy.Service(
            '%s/getAngle' % config['rostopicName'], nicomsg.srv.GetValue,
            self._ROSPY_getAngle)
        rospy.Service(
            '%s/getPose' % config['rostopicName'], nicomsg.srv.GetValues,
            self._ROSPY_getPose)
        rospy.Service('%s/getJointNames' %
                      config['rostopicName'], nicomsg.srv.GetNames,
                      self._ROSPY_getJointNames)
        rospy.Service('%s/getAngleUpperLimit' %
                      config['rostopicName'], nicomsg.srv.GetValue,
    ...
    def _sendJointState(self):
        ...
        while self._running:
            ...
            for joint in joints:
                if (self.config['fakeExecution']
                        and joint in self.fakeJointStates):
            ...
            joints_without_motor = ['l_indexfinger_1st_x',
                                    'l_indexfinger_2nd_x', 'l_ringfingers_x',
                                    'l_ringfinger_1st_x', 'l_ringfinger_2nd_x',
                                    'l_thumb_1st_x', 'l_thumb_2nd_x',
                                    'r_indexfinger_1st_x',
                                    'r_indexfinger_2nd_x', 'r_ringfingers_x',
                                    'r_ringfinger_1st_x', 'r_ringfinger_2nd_x',
                                    'r_thumb_1st_x', 'r_thumb_2nd_x']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log-file', dest='logFile',
                        help=(
                            'Path to log file. Default: %s' % config['logFile']
                        ), type=str)
    parser.add_argument('-m', '--motor-file', dest='robotMotorFile',
                        help=(
                            'Path to robot motor file. Default: ' +
                            config['robotMotorFile']), type=str)
    ...
    parser.add_argument('--vrep-scene', dest='vrepScene',
                        help=(
                            'Scene to load in VREP. Default: ' +
                            config['vrepScene']), type=str)
    parser.add_argument('--rostopic-name', dest='rostopicName',
                        help=('Topic name for ROS. Default:' +
                              config['rostopicName']), type=str)
    parser.add_argument('-j', '--joint-state-name', dest='jointStateName',
                        ...
                        ) % config['jointStateName'], type=str)
