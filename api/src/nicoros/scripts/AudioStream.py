...
class AudioStream:
    def __init__(self):
        '''AudioStream allows to stream audiodata received from a microphone
        via ROS'''
        ...
        self._startSrv = rospy.Service(
            'nico/audiostream/startStream', nicomsg.srv.StartAudioStream,
            self._ROSPY_startStream)
        self._stopSrv = None
        # actual streaming
        sampleIndex = 0
        while not rospy.is_shutdown():
            if (self._running
                    and self._recorder.get_number_of_samples() > sampleIndex):
                sample = self._recorder.get_chunk(sampleIndex)
                sampleIndex += 1
                msg = nicomsg.msg.hs()
                msg.header.stamp = rospy.Time.now()
                if self._channels[0]:
                    msg.param1 = audioop.tomono(
                        sample, self._recorder.get_sample_width(), 1, 0)
                    self._publisherLeft.publish(msg)
                if self._channels[1]:
                    msg.param1 = audioop.tomono(
                        sample, self._recorder.get_sample_width(), 0, 1)
    ...
    def startStream(self, filename, samplerate, channels):
        ...
        if channels[0]:
            self._publisherLeft = rospy.Publisher(
                'nico/audiostream/left', nicomsg.msg.hs, queue_size=10)
        if channels[1]:
            self._publisherRight = rospy.Publisher(
                'nico/audiostream/right', nicomsg.msg.hs, queue_size=10)
        # start runtime services
        self._getWidthSrv = rospy.Service(
            'nico/audiostream/getSampleWidth', nicomsg.srv.GetIntValue,
            self._ROSPY_getSampleWidth)
        self._getRateSrv = rospy.Service(
            'nico/audiostream/getFrameRate', nicomsg.srv.GetIntValue,
            self._ROSPY_getFrameRate)
        self._stopSrv = rospy.Service(
            'nico/audiostream/stopStream', nicomsg.srv.StopAudioStream,
            self._ROSPY_stopStream)
...
# ----------------------------Service Callbacks--------------------------------
    ...
    def _ROSPY_startStream(self, msg):
        ...
        :rtype: bool
        """
        return self.startStream(msg.filename, msg.samplerate, (msg.left,
                                                               msg.right))
    ...
    def _ROSPY_getSampleWidth(self, _):
        """
        ROS service handle to get the sample width of the stream (needed for
        wave)
    ...
    def _ROSPY_getFrameRate(self, _):
        """
        ROS service handle to get the sampling frequency of the stream (needed
        for wave)
