...
class AudioReceiver:
    ...
    def __init__(self):
        '''Example class for receiving audiostreams and synchronizing the
        ...
        try:
            getSampleWidth = rospy.ServiceProxy(
                'nico/audiostream/getSampleWidth', nicomsg.srv.GetIntValue)
            getFrameRate = rospy.ServiceProxy(
        ...
        left = message_filters.Subscriber(
            "nico/audiostream/left", nicomsg.msg.hs)
        right = message_filters.Subscriber(
            "nico/audiostream/right", nicomsg.msg.hs)
