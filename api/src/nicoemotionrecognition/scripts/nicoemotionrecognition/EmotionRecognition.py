...
class EmotionRecognition:
    def __init__(self, device='', robot=None, face=None, faceDetectionDelta=10, voiceEnabled=False, german=False):
        """
    ...
    def getHighestMatchingEmotion(self):
        ...
        if self._categoricalRecognition is not None:
            ...
            if self._categoricalRecognition[0][6] > 15:
                ...
                max_index = numpy.argmax(self._categoricalRecognition[0])
                max_classname = self._modelCategorical.modelDictionary.classsesOrder[max_index]
                # print "I will return: " + str(self._modelCategorical.modelDictionary.classsesOrder[numpy.argmax(self._categoricalRecognition[0])].lower())
            return self._modelCategorical.modelDictionary.classsesOrder[numpy.argmax(self._categoricalRecognition[0])].lower()
        return None
            ...
            if not len(face) == 0:
                ...
                if self._faceTracking and self._trackingCounter == 0:
                    if self._robot is not None:
                        ...
                        # horizontal
                        angle_z = (
                            640-facePoints[0].center().x)/640.0 * 60 - 60/2.0
                        # vertikal
                        angle_y = (
                            480-facePoints[0].center().y)/480.0 * 50 - 50/2.0
                ...
                with self._graph.as_default():
                    self._categoricalRecognition = self._modelCategorical.classify(
                        face)
                    self._dimensionalRecognition = self._modelDimensional.classify(
                        face)
					...
					if self.last_exp != self.getHighestMatchingEmotion():
						...
						if self._voiceEnabled and self._same_emotion_counter == 3:                              
							if self.last_exp == "happiness":
								import random
								if self._german:
									sents = ["Ich bin froehlich, wenn Du es auch bist!",
										"Was fuer ein schoener Tag heute ist, nicht wahr?", "Du bist gerade gluecklich, stimmts?"]
								else:
									sents = ["I am happy, if YOU are happy.",
										"What a nice day, right?", "You are happy right now, are you?"]
								say(random.choice(sents))
							if self.last_exp == "surprise":
								import random
								if self._german:
									sents = ["Du siehst ueberrascht aus. Was ist denn los?",
										"Bist Du ueberrascht, was fuer ein smarter Roboter ich bin?", "Das ist eine Ueberraschung, nicht wahr?"]
								else:
									sents = ["You look surprised. Is everything alright?",
										"Are you surprised, what a smart robot I am?", "This is a surprise, right?"]
								say(random.choice(sents))
							if self.last_exp == "anger":
								import random
								if self._german:
									sents = ["Du siehst so aergerlich aus, ist alles in Ordnung?",
										"aeh Digga, ich bin auch sauer!", "Was ist Dir denn ueber die Leber gelaufen?"]	
								else:
									sents = ["You look angry. Is everything alright?",
										"I am angry as well!", "What went wrong?"]
								say(random.choice(sents))
							if self.last_exp == "fear":
								import random
								if self._german:
									sents=["Hast Du vor etwas Angst? Was ist denn hier gefaehrlich?","Ich bin ganz harmlos! Du musst keine Angst vor mir haben!",
											"Du siehst aus, als haettest Du einen Geist gesehen! Da bekomme ich auch Angst"]
								else:
									sents=["Do you fear something?. What is dangerous here?","I am harmless! You do not have to fear me",
