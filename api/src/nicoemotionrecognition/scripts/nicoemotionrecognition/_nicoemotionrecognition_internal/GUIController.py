...
class GUIController:
    def createDetectedFacGUI(
        self,
        frame,
        detectedFace,
        modelDictionary=None,
    ...
    def createDimensionalEmotionGUI(
        self,
        ...
        # arousal,valence
        cv2.circle(
            frame, (640 + 185 + int(valence), 110 + int(arousal)), 5, pointColor, -1
        )

        return frame

    def createCategoricalEmotionGUI(
        self, classificationReport, frame, modelDictionary, initialPosition=0
    ):
        ...
        for index, emotion in enumerate(modelDictionary.classsesOrder):
            ...
            cv2.putText(
                frame,
                emotion,
                (640 + 5, initialPosition + 15 + int(index) * 20),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                modelDictionary.classesColor[index],
                1,
            )

            cv2.rectangle(
                frame,
                (640 + 100, initialPosition + 5 + int(index) * 20),
                (
                    int(640 + 100 + emotionClassification),
                    initialPosition + 20 + int(index) * 20,
                ),
                modelDictionary.classesColor[index],
                -1,
            )
            cv2.putText(
                frame,
                str(emotionClassification) + "%",
                (
                    int(640 + 105 + emotionClassification + 10),
                    initialPosition + 20 + int(index) * 20,
                ),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
