...
class Chain(object):
    ...
    def __repr__(self):
        return (
        "Kinematic chain name={} links={} active_links={}".format(self.name, self.links, self.active_links_mask))
    ...
    def inverse_kinematics(self, target, initial_position=None, second_chain=None, second_target_frame=None,
                           method="L-BFGS-B", orientation_weight=None, **kwargs):
        ...
        if (method == "L-BFGS-B") or (method == "SLSQP"):

            if initial_position is None:
                initial_position = [0] * len(self.links)

            return ik.inverse_kinematic_optimization(self, target, starting_nodes_angles=initial_position,
        ...
        if (method == "multi"):
                ...
                return ik.inverse_kinematic_optimization_multi(self, target, starting_nodes_angles=initial_position,
                                                         method=method, **kwargs)

        # genetic algoritm methods
        if (method == "ga_simple"):
            return ik.inverse_kinematic_ga(self, target, starting_nodes_angles=initial_position,
                                           second_chain=second_chain, second_target_frame=second_target_frame,
                                           method=method, orientation_weight=orientation_weight, **kwargs)

        # ccd methods
        if (method == "ccd_simple"):
            # if initial_position is None:
            #	initial_position = [0.0] * len(self.links)
            return ik.inverse_kinematic_ccd(self, target, starting_nodes_angles=initial_position,
                                            second_chain=second_chain, second_target_frame=second_target_frame,
                                            method=method, orientation_weight=orientation_weight, **kwargs)
    ...
    @classmethod
    def from_urdf_file(cls, urdf_file, base_elements=["base_link"], last_link_vector=None, base_element_type="link",
                       active_links_mask=None, name="chain", last_link=None):
       ...
       :param string Name: The name of the Chain
        """
        links = URDF_utils.get_urdf_parameters(urdf_file, base_elements=base_elements,
                                               last_link_vector=last_link_vector, base_element_type=base_element_type,
                                               last_link=last_link)
    ...
    def get_all_active_link_names(self):
        link_names = [link.name for link in self.active_from_full(self.links)]
        return (link_names)

    @classmethod
    def concat(cls, chain1, chain2):
        return cls(links=chain1.links + chain2.links,
                   active_links_mask=chain1.active_links_mask + chain2.active_links_mask)
