...
class Mover:
	...
	def record_movement(self,fname=None):
		...
		with open(fname,"wb") as jointfile:
			...
			while (key!="q"):	
				...
				recent_joint_positions=[self.robot.getAngle(jName) for jName in (self.robot.getJointNames())]
	...
	def move_position(self, target_positions,speed,real=True):
		...
		time_to_reach = {k: abs((float(current_positions[k]) - float(target_positions[k]))/cur_speed) for k in current_positions}
		#print time_to_reach
		...
		for joi in target_positions:
			if real and max_time != 0.0:
				self.robot.setAngle(joi, float(target_positions[joi]), (speed*time_to_reach[joi])/max_time)
		return max_time
	...
	def calc_move_file(self, fname, target_fname, number):
		...
		with open(fname, 'rb') as infile:
			...
			for joint in current_positions:
				current_positions[joint] = self.robot.getAngle(joint)
			stepsize = {k: (float(target_positions[k]) - float(current_positions[k]) ) / number for k in
							 current_positions}
...
if __name__ == "__main__":
	...
	parser.add_argument("command", help="One of the commands m (record movement), p (record position), pm (play movement), pp (play position), cm (calculate movement) fj(freeze joints) uj(unfreeze joints)")
	#fj freeze joints as they are by torquing it. You subset to freeze only a subset of the joints.
	parser.add_argument('--json', nargs='?', default='../../../../../json/nico_humanoid_upper.json', help="robots json file. Default: nico_humanoid_upper.json")
	parser.add_argument('--filename', nargs='?', default=None, help="file to record or to play")
	parser.add_argument('--targetfilename', nargs='?', default="/tmp/mov-calc.csv", help="file to write")
	...
	parser.add_argument('--vrep', action="store_true", default=False, help="let it run on vrep than instead of real robot")
