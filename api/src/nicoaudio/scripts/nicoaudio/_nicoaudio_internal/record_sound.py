...
class RecordSound(threading.Thread):
    ...
    def __init__(self,
                 device_name='AudioBox USB 96: Audio (hw:2,0)',
                 has_threshold=False,
                 chunk_size=256,
        ...
        Args:
            device_name:    Optional string specifying the name of the
                            recording device.
    ...
    def _device_index(self):
        ...
        if self._device_name in self.get_devices():
            return list(filter(lambda device: device['name'] ==
                               self._device_name,
                               map(self._pyAudio.get_device_info_by_index,
    ...
    def get_chunk(self, index=-1):
        """Returns specific chunk of streamed data. (default: current chunk)"""
    ...
    def save(self, filename='sound.wav', activeChannels=(True, True)):
        ...
        Args:
            ...
            activeChannels: An optional boolean tuple to save only specific
                            channels of the signal (left,right)
        """
        ...
        if channels == 2:
            ...
            wf.writeframes(b''.join(map(
                                    lambda sample: audioop.tomono(
                                        sample, self.get_sample_width(),
