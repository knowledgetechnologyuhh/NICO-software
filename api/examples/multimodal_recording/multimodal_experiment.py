...
def get_needed_numbers_for_object(object_name,action):
    ...
    num = number_of_samples_per_object - get_sampled_numbers_for_object(object_name,action)
    return (num)
...
while ( get_needed_overall_numbers() > 0 ):
	...
	for n in range(len(actions[action)]):
    ...
    print "\n Waiting for pictures writing on disk"
    ...
    if not data_check_clean(cur_dir,dfl,dfr):
        answer="R"
