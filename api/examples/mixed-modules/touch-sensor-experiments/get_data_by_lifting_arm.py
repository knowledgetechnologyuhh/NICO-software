...
objects = ["red_tomato", "green_sausage", "red_ball", "yellow_banana", "red_banana", "yellow_dice", "green_pepper", "blue_ball",
           "red_dice", "puple_grapes", "red_sponge", "orange_carrot", "black_hat", "purple_duck", "orange_fish", "green_figure"]
...
def get_needed_numbers_for_object(object_name):
    ...
    num = number_of_samples_per_object - \
        get_sampled_numbers_for_object(object_name)
    return (num)
...
def print_progress():
    for o in objects:
        print " For " + o + " - samples needed: " + \
            str(get_needed_numbers_for_object(o))
...
while (get_needed_overall_numbers() > 0):
    ...
    with pypot.dynamixel.DxlIO('/dev/ttyACM0') as dxl_io:
        ...
        for it, pos in enumerate(range(-180, 80, 10)):
            ...
            #    try:
            id = 31
            cur_thumb = dxl_io.get_present_motor_currents([id])[0][2]
            print "Motor Thumb Current " + str(cur_thumb) + " id " + str(id)
            cur_finger = dxl_io.get_present_motor_currents([id])[0][3]
            ...
            print str(time_touch) + "," + str(counter) + "," + str(sensor_status) + \
                "," + str(x) + "," + str(y) + "," + \
                str(z) + "," + str(checksum)
            ...
            pygame.image.save(img, "./arm_data/" +
                              str(subsample_number) + ".jpg")
    ...
    for n in range(1, 9, 1):
        ...
        sql_command = format_str_subsample_arm.format(sample_number=sample_number, subsample_iteration=n,
                                                      r_elbow_y_position=robot.getAngle(
                                                          "r_elbow_y"),
                                                      r_arm_x_position=robot.getAngle(
                                                          "r_arm_x"),
                                                      r_shoulder_z_position=robot.getAngle(
                                                          "r_shoulder_z"),
                                                      r_shoulder_y_position=robot.getAngle(
                                                          "r_shoulder_y"),
                                                      r_elbow_y_current=robot.getCurrent(
                                                          "r_elbow_y"),
                                                      r_arm_x_current=robot.getCurrent(
                                                          "r_arm_x"),
                                                      r_shoulder_z_current=robot.getCurrent(
                                                          "r_shoulder_z"),
                                                      r_shoulder_y_current=robot.getCurrent(
                                                          "r_shoulder_y"),
        ...
        pygame.image.save(img, "./arm_data/arm_" +
                          str(subsample_number) + ".jpg")
        # call(["fswebcam", "-r", "640x480", "-d", "/dev/video0", "--jpeg", "95", "-D", "1",
        #      "./data/" + str(subsample_number) + ".jpg"])
        sleep(.1)
