...
from stub_touch_data_classifier_stop import \
    touch_data_classifier as stop_touch_data_classifier
...
classifier = RemoteObject(wrapped_function=wrapped_function,  # the function that initializes the remote object
                          ...
                          path_to_virtualenv="../../../../../../NICO/NICO-haptic-object-classification/virtualenv",
                          ...
                          original_working_directory="../../../../../../NICO/NICO-haptic-object-classification/",
                          ...
                          flaskcom_path="/home/sysadmin/NICO",
...
def get_present_thumb_current_safe(dxo, joint):
    ...
    while not succes:
        try:
            pos = dxo.get_present_motor_currents(joint)[0][2]
...
def get_present_finger_current_safe(dxo, joint):
    ...
    while not succes:
        try:
            pos = dxo.get_present_motor_currents(joint)[0][3]
    ...
    if not repeat:
        ...
        with pypot.dynamixel.DxlIO('/dev/ttyACM1') as dxl_io:
            ...
            sql_command = format_str_sample.format(
                object_name="Unknown", timecode=datetime.datetime.now().isoformat())
            #print "\n " +sql_command
            ...
            for it, pos in enumerate(range(-180, 80, 5)):
                ...
                cur_thumb = get_present_thumb_current_safe(
                    dxl_io, [id]) / thumb_force_calibrator

                #print "Motor Thumb Current " + str(cur_thumb) + " id " + str(id)
                cur_finger = get_present_finger_current_safe(
                    dxl_io, [id]) / finger_force_calibrator
                ...
                (time_touch, counter, sensor_status, x, y,
                 z, checksum) = optsens.get_sensor_all()
                ...
                if ((cur_thumb > MAX_CUR_THUMB) or (cur_finger > MAX_CUR_FINGER)) and status == 0:
                    ...
                    print "force_calibrators (thumb/fingers): " + str(
                        thumb_force_calibrator) + " / " + str(finger_force_calibrator)
                    print "MAX_CUR_THUMB / cur_thumb : " + \
                        str(MAX_CUR_THUMB) + "/" + str(cur_thumb)
                    print "MAX_CUR_FINGER / cur_finger : " + \
                        str(MAX_CUR_FINGER) + "/" + str(cur_finger)
        ...
        to_say = {"green_sausage": "cucumber", "red_tomato": "tomato",
                  "yellow_banana": "banana", "red_ball": "ball"}
